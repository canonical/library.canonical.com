        ...
        if change.get("removed"):
            ...
            if "fileId" in change and navigation_data:
                if change["fileId"] in navigation_data.doc_reference_dict:
                    ...
                    if nav_item.get("full_path") != new_nav_item.get(
                        "full_path"
                    ):
...
def scheduled_get_changes():
    ...
    state.nav_changes = process_changes(
        changes, state.nav_changes, state.gdrive_instance
    )
...
def init_scheduler(app, document_fn):
    ...
    def ingest_all_documents_job():
        ...
        with app.app_context():
            try:
                    ...
                    with app.app_context():
                        try:
                            ...
                            get_or_parse_document(
                                drive, doc_id, doc_dict, meta.get("name", "")
                            )
                ...
                msg1 = f"[ingest] done created={created} skipped={skipped}"
                msg2 = f"[ingest] done errors={errors} total={total}"
    ...
    def scheduled_clean_cache():
        if not os.getenv("REDIS_DB_CONNECT_STRING"):
            print(
                "[cache] Redis not configured; skipping cache clean",
                flush=True,
            )
            return
        ...
        if not os.path.exists(url_file_path):
            with app.app_context():
                try:
                    ...
                    print(
                        f"[cache] failed to construct navigation: {e}",
                        flush=True,
                    )
        ...
        print(
            f"[cache] cleared {removed} view entries from url_list", flush=True
        )
